...
\section{Exigences du projet}
	...
	\begin{description}
		...
		\item [Vendredi 19/Juin - 5 Points]  Livraison d'une révision de la conception UML suite à l'implémentation et d'un court rapport expliquant les raisons de l'évolution de l'UML s'il y en a eu.
		\item [Lundi 19/Juin - 5 Points] Implémentation complète de la simulation. 
	\end{description} 
	
	
	\paragraph{Sur la Conception} 
		La conception du projet s'agit d'une proposition de solution pour répondre a l'exercise. 
		Il s'agit de un modele UML de classes, la implementation en pseudo code de la fonction de simulation d'un tour de rôle, et en rédigeant un rapport de conception qu'explique les classes choisi, les types structurel choisi (List, arrays, etc) et que propose des sujets important a tester unitairement pendant le développement, en expliquent la raison du test.
		
		
	\paragraph{Sur la l'implémentation de la création du plateau}
		C'est le premier point formel du travail,  On est sense a presenter le code nécessaire pour créer un plateau de jeu et l'utiliser. 
